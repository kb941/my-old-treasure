...
export function useReadinessScore(input: ReadinessInput): ReadinessResult {
  ...
  const result = useMemo(() => {
    ...
    if (hasStudyLogs) {
      ...
      if (daysSinceStudy >= 7) { inactive = -3; penaltyDetails.push(`${daysSinceStudy} days inactive! Resume immediately`); }
      else if (daysSinceStudy >= 3) { inactive = -1; penaltyDetails.push(`${daysSinceStudy} days since last study`); }
    }
